...
<div align="right">
<a href="<?= base_url('Pesanan/index_pesanan'); ?>"class="btn btn-danger btn-sm">Kembali</i></a>

 <?php if($invoice->Status_bayar == 5) {?>
                                
    <?php } else{?>
        <a href="<?= base_url('Pesanan/after_bayar/'.$invoice->id_pesanan); ?>"class="btn btn-primary btn-sm" onclick="return confirm('Apakah Data Konsumen ini Sudah Benar?')">Proses Bayar</i></a>
        <?php }?>
</div>
    </div>
         </div>
